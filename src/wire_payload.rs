...
/// Fixed header size (no KEM ciphertext): 46 bytes.
pub const HEADER_SIZE: usize =
    MSG_NUM_SIZE + DH_KEY_SIZE + OTPK_ID_SIZE + KYBER_OTPK_ID_SIZE + KEM_LEN_SIZE;
...
mod tests {
    ...
    fn round_trip_no_pqc() {
        ...
        let packed = pack(&dh_key, 7, 42, 0, None, &sealed).unwrap();
    ...
    fn round_trip_with_pqc() {
        ...
        let packed = pack(&dh_key, 0, 99, 5, Some(&kem_ct), &sealed).unwrap();
    ...
    fn invalid_dh_key_size() {
        let err = pack(&[0u8; 16], 0, 0, 0, None, &make_sealed_box(0)).unwrap_err();
        assert!(matches!(err, WirePayloadError::InvalidDhPublicKey(16)));
    }

    /// Verify byte-level compatibility with Swift WirePayloadCoder.
    /// Known vector: msgNum=1, dh=0x01×32, otpkId=2, kyberOtpkId=0, no PQC, sealed=0xAA×60
    #[test]
    fn known_byte_vector() {
        let dh_key = vec![0x01; 32];
        let sealed = vec![0xAA; 60];
        let packed = pack(&dh_key, 1, 2, 0, None, &sealed).unwrap();
        ...
        // sealed box
        assert_eq!(&packed[46..], vec![0xAAu8; 60].as_slice());
    }
}
