...
/// Debug-mode guard: `new_initiator_session` must panic when `local_user_id`
/// is a 32-char hex string (device-hash format) with no dashes.
#[cfg(debug_assertions)]
#[test]
#[should_panic(expected = "looks like a 32-char device-hash")]
fn test_debug_assert_rejects_device_hash_as_local_user_id() {
    let (alice_priv, _alice_pub) = ClassicSuiteProvider::generate_kem_keys().unwrap();
    let (bundle, _bob_priv, _bob_spk_priv, bob_pub) = make_bob_bundle();

    let (rk_alice, init_state) =
        X3DHProtocol::<ClassicSuiteProvider>::perform_as_initiator(&alice_priv, &bundle).unwrap();

    // This must panic: local_user_id = 32-char hex without dashes.
    let _ = DoubleRatchetSession::<ClassicSuiteProvider>::new_initiator_session(
        &rk_alice,
        init_state,
        &bob_pub,
        "81f02199-8374-48f8-8a5f-549434ccc53f".to_string(), // contact_id: UUID ✓
        "6f5e37ac88bd2cc53348f01f78cdf5db".to_string(),     // local_user_id: hash ✗
    );
}

/// Debug-mode guard: `new_initiator_session` must panic when `contact_id`
/// is a 32-char hex string (device-hash format) with no dashes.
#[cfg(debug_assertions)]
#[test]
#[should_panic(expected = "looks like a 32-char device-hash")]
fn test_debug_assert_rejects_device_hash_as_contact_id() {
    let (alice_priv, _alice_pub) = ClassicSuiteProvider::generate_kem_keys().unwrap();
    let (bundle, _bob_priv, _bob_spk_priv, bob_pub) = make_bob_bundle();
    ...
    // contact_id is a device-hash — this is also wrong.
    let _ = DoubleRatchetSession::<ClassicSuiteProvider>::new_initiator_session(
        &rk_alice,
        init_state,
        &bob_pub,
        "6f5e37ac88bd2cc53348f01f78cdf5db".to_string(), // contact_id: hash ✗
        "14f28d31-2dab-44aa-a123-456789abcdef".to_string(), // local_user_id: UUID ✓
    );
}

/// debug_assert must also fire in `new_responder_session`.
#[cfg(debug_assertions)]
#[test]
#[should_panic(expected = "looks like a 32-char device-hash")]
fn test_debug_assert_responder_rejects_device_hash_as_local_user_id() {
    let (alice_priv, alice_pub) = ClassicSuiteProvider::generate_kem_keys().unwrap();
    let (bundle, bob_priv, bob_spk_priv, bob_pub) = make_bob_bundle();
    ...
    let mut alice = DoubleRatchetSession::<ClassicSuiteProvider>::new_initiator_session(
        ...
        init_state,
        &bob_pub,
        "81f02199-8374-48f8-8a5f-549434ccc53f".to_string(),
        "14f28d31-2dab-44aa-a123-456789abcdef".to_string(),
    )
    .unwrap();
    let msg0 = alice.encrypt(b"responder guard test").unwrap();
    ...
    // Bob passes a device-hash as local_user_id → debug_assert fires.
    let _ = DoubleRatchetSession::<ClassicSuiteProvider>::new_responder_session(
        &rk_bob,
        &bob_priv,
        &msg0,
        "14f28d31-2dab-44aa-a123-456789abcdef".to_string(),
        "6f5e37ac88bd2cc53348f01f78cdf5db".to_string(), // device-hash ✗
    );
}
